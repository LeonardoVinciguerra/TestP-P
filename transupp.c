...
jtransform_parse_crop_spec (jpeg_transform_info *info, const char *spec)
{
	...
	//GF_30_11_2012
	/* if empty reset crop parameters */
	if( *spec == '\0' )
		return TRUE;
			      ...
			      jpeg_transform_info *info)
{
  ...
  if (need_workspace) {
    ...
    width_in_iMCUs = (JDIMENSION)
      jdiv_round_up((int) info->output_width,
		    (int) info->iMCU_sample_width);
    height_in_iMCUs = (JDIMENSION)
      jdiv_round_up((int) info->output_height,
		    (int) info->iMCU_sample_height);
			...
			JDIMENSION new_width, JDIMENSION new_height)
{
  ...
  do {
	...
	new_value = new_height; /* ExifImageHeight Tag */
      if (is_motorola) {
	data[offset+2] = 0; /* Format = unsigned int (4 octets) */
	data[offset+3] = 4;
	...
	data[offset+2] = 4; /* Format = unsigned int (4 octets) */
	data[offset+3] = 0;
